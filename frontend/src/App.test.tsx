...
describe('Iteration 04 integration state', () => {
  ...
  it('previews all pending positions without mutating the active plan and cancels cleanly', async () => {
    ...
    expect(legend).toHaveTextContent('Текущие даты')
    expect(legend).toHaveTextContent('Запрошенное изменение')
    expect(legend).toHaveTextContent('Сдвиг по зависимости')
    expect(legend).not.toHaveTextContent('После применения')
